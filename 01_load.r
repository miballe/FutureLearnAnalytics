...
#
# Known Issues: Exceptionally, during a test a file with a NUL entry was found
...
load_local_csv <- function(data_folder, file_name) {
  ...
  if(file.exists(file_path)) {
    ...
    log_new_debug(paste("- READING - Local CSV", file_path))
    df_return <- read.csv(file_path, stringsAsFactors=FALSE)
    frstop_time <- proc.time() - frstart_time
    log_new_debug(paste("- COMPLETE - Local CSV", file_path, "- Elapsed:", frstop_time[3], "s"))
  } else {
    log_new_warning(paste("The file", file_path, "doesn't exist. This might be expected, but please double-check."))
  }
  return(df_return)
}
...
load_merge_downloaded_csvs <- function(short_ids, data_folder, suffix) {
  ...
  for(short_id in short_ids) {
    ...
    if(file.exists(file_path)) {
      frstart_time <- proc.time()
      log_new_debug(paste("- READING - Downloaded CSV", file_path))
      ...
      frstop_time <- proc.time() - frstart_time
      log_new_debug(paste("- COMPLETE - Downloaded CSV", file_path, "- Elapsed:", frstop_time[3], "s"))
    } else {
      log_new_warning(paste("The file", file_path, "doesn't exist. This might be expected, but please double-check."))
    }
  }
...
save_local_csv <- function(data_frame, file_name, data_folder) {
  frstart_time <- proc.time()
  file_path <- paste("./", data_folder, "/", file_name, ".csv", sep = "")
  log_new_debug(paste("- WRITING - Local File", file_path))
  write.csv(data_frame, file_path, row.names = FALSE)
  frstop_time <- proc.time() - frstart_time
  log_new_debug(paste("- COMPLETE - Local File", file_path, "- Elapsed:", frstop_time[3], "s"))
}
...
load_from_csv_course_list <- function() {
  ...
  log_new_info("- START - load_from_csv_course_list")

  course_list <- load_local_csv(DATA_DOWNLOADED, FILE_COURSELIST)

  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_from_csv_course_list - Elapsed:", fstop_time[3], "s"))
  return(course_list)
}
...
load_from_csv_course_details <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_from_csv_course_details")

  course_details <- load_local_csv(DATA_DOWNLOADED, FILE_COURSEDETAILS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_from_csv_course_details - Elapsed:", fstop_time[3], "s"))
...
load_downloaded_comments <- function(code_list) {
  fstart_time <- proc.time()
  log_new_info("- START - load_downloaded_comments")

  dwnld_comments <- load_merge_downloaded_csvs(code_list, DATA_DOWNLOADED, SUFFIX_COMMENTS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_downloaded_comments - Elapsed:", fstop_time[3], "s"))
...
load_downloaded_enrolments <- function(code_list) {
  fstart_time <- proc.time()
  log_new_info("- START - load_downloaded_enrolments")

  dwnld_enrolments <- load_merge_downloaded_csvs(code_list, DATA_DOWNLOADED, SUFFIX_ENROLMENTS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_downloaded_enrolments - Elapsed:", fstop_time[3], "s"))
...
load_downloaded_peer_review_assignments <- function(code_list) {
  fstart_time <- proc.time()
  log_new_info("- START - load_downloaded_peer_review_assignments")

  dwnld_peer_review_assignments <- load_merge_downloaded_csvs(code_list, DATA_DOWNLOADED, SUFFIX_PRASSIGNMENTS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_downloaded_peer_review_assignments - Elapsed:", fstop_time[3], "s"))
...
load_downloaded_peer_review_reviews <- function(code_list) {
  fstart_time <- proc.time()
  log_new_info("- START - load_downloaded_peer_review_reviews")

  dwnld_peer_review_reviews <- load_merge_downloaded_csvs(code_list, DATA_DOWNLOADED, SUFFIX_PRREVIEWS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_downloaded_peer_review_reviews - Elapsed:", fstop_time[3], "s"))
...
load_downloaded_question_response <- function(code_list) {
  fstart_time <- proc.time()
  log_new_info("- START - load_downloaded_question_response")

  dwnld_question_response <- load_merge_downloaded_csvs(code_list, DATA_DOWNLOADED, SUFFIX_QUESTIONRESPONSE)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_downloaded_question_response - Elapsed:", fstop_time[3], "s"))
...
load_downloaded_step_activity <- function(code_list) {
  fstart_time <- proc.time()
  log_new_info("- START - load_downloaded_step_activity")

  dwnld_step_activity <- load_merge_downloaded_csvs(code_list, DATA_DOWNLOADED, SUFFIX_STEPACTIVITY)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_downloaded_step_activity - Elapsed:", fstop_time[3], "s"))
...
save_raw_course_list <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_course_list")
  if(exists("df_to_save")) {
    save_local_csv(df_to_save, FILE_COURSELIST, DATA_RAW)
  } else {
    log_new_error("The Data Frame to save doesn't exist!")
  }
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_course_list - Elapsed:", fstop_time[3], "s"))
}

save_raw_course_details <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_course_details")
  if(exists("df_to_save")) {
  ...
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_course_details - Elapsed:", fstop_time[3], "s"))
}

save_raw_comments <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_comments")
  if(exists("df_to_save")) {
  ...
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_comments - Elapsed:", fstop_time[3], "s"))
}

save_raw_enrolments <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_enrolments")
  if(exists("df_to_save")) {
  ...
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_enrolments - Elapsed:", fstop_time[3], "s"))
}

save_raw_pr_assignments <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_pr_assignments")
  if(exists("df_to_save")) {
  ...
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_pr_assignments - Elapsed:", fstop_time[3], "s"))
}

save_raw_pr_reviews <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_pr_reviews")
  if(exists("df_to_save")) {
  ...
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_pr_reviews - Elapsed:", fstop_time[3], "s"))
}

save_raw_question_response <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_question_response")
  if(exists("df_to_save")) {
  ...
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_question_response - Elapsed:", fstop_time[3], "s"))
}

save_raw_step_activity <- function(df_to_save) {
  fstart_time <- proc.time()
  log_new_info("- START - save_raw_step_activity")
  if(exists("df_to_save")) {
  ...
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - save_raw_step_activity - Elapsed:", fstop_time[3], "s"))
}
...
load_raw_course_list <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_course_list")
  
  raw_course_list <- load_local_csv(DATA_RAW, FILE_COURSELIST)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_course_list - Elapsed:", fstop_time[3], "s"))
  return(raw_course_list)
}

load_raw_course_details <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_course_details")
  
  raw_course_details <- load_local_csv(DATA_RAW, FILE_COURSEDETAILS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_course_details - Elapsed:", fstop_time[3], "s"))
  return(raw_course_details)
}

load_raw_comments <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_comments")
  
  raw_comments <- load_local_csv(DATA_RAW, SUFFIX_COMMENTS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_comments - Elapsed:", fstop_time[3], "s"))
  return(raw_comments)
}

load_raw_enrolments <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_enrolments")
  
  raw_enrolments <- load_local_csv(DATA_RAW, SUFFIX_ENROLMENTS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_enrolments - Elapsed:", fstop_time[3], "s"))
  return(raw_enrolments)
}

load_raw_peer_review_assignments <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_peer_review_assignments")
  
  raw_pr_assignments <- load_local_csv(DATA_RAW, SUFFIX_PRASSIGNMENTS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_peer_review_assignments - Elapsed:", fstop_time[3], "s"))
  return(raw_pr_assignments)
}

load_raw_peer_review_reviews <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_peer_review_reviews")
  
  raw_pr_reviews <- load_local_csv(DATA_RAW, SUFFIX_PRREVIEWS)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_peer_review_reviews - Elapsed:", fstop_time[3], "s"))
  return(raw_pr_reviews)
}

load_raw_question_response <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_question_response")
  
  raw_question_response <- load_local_csv(DATA_RAW, SUFFIX_QUESTIONRESPONSE)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_question_response - Elapsed:", fstop_time[3], "s"))
  return(raw_question_response)
}

load_raw_step_activity <- function() {
  fstart_time <- proc.time()
  log_new_info("- START - load_raw_step_activity")
  
  raw_step_activity <- load_local_csv(DATA_RAW, SUFFIX_STEPACTIVITY)
  
  fstop_time <- proc.time() - fstart_time
  log_new_info(paste("- END - load_raw_step_activity - Elapsed:", fstop_time[3], "s"))
